...
Turn on **Log each dose** for a medication to track doses one by one. You define up to three dose slots with a label and amount, for example AM 1 and PM 0.5. The card then shows a button per slot for today, plus three quick amount buttons: 1/2, 1, and 1.5. Tap a slot or an amount, then press OK to log it. Closing the confirmation any other way logs nothing. Tap a taken slot again to undo it. Pills per day is set from the slot amounts. With dose logging on, the count only goes down by the doses you log, so recount now and then if you forget to tap. With it off, the count drops by pills per day automatically and Dose entries are a record only.
