...
Turn on **Log each dose** for a medication to track doses one by one. You define up to three dose slots with a label and amount, for example AM 1 and PM 0.5. The card then shows a button per slot for today, plus an Other link that reveals three quick amounts: 1/2, 1, and 1.5. One tap logs the dose and a toast offers Undo for a few seconds. Tapping a taken slot again asks before removing it. Pills per day stays editable as the planned rate for days left and surplus, so you can set 2 when the buttons are AM 0.5, PM 1, and PM 1.5. Leave it blank to use the slot total. With dose logging on, the count only goes down by the doses you log, so recount now and then if you forget to tap. With it off, the count drops by pills per day automatically and Dose entries are a record only.
